...
if (path_array.length == 3) {
   mode = path_array[1];
   waypoint = path_array[2];
}

if (mode == "departures") {
   URL = "/getJsonPDepartures";
} else if (mode == "arrivals") {
   URL = "/getJsonPArrivals";
}
...
function addSolariBoard(divSelector) {
    ...
    var $solari = $("<div class=\"column solari_grid\">" +
            ...
            "<li class=\"departure\"> " + arrival_departure + "</li>" +
...
function updateSolariBoard() {
    ...
    $.getJSON(URL + (URL.indexOf("?") === -1 ? '?' : '&') + "callback=?" + URL_SUFFIX, function(new_board) {
            ...
            if (mode == "arrivals") {
                $("ul.solari-board-columns li.departure").text("Arrival");
            } else {
